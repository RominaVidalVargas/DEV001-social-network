...
const inicio = (changeRouter2) => {
  ...
  Btning.addEventListener("click", () => {
    changeRouter2("/ingresar");
  });
  crear.addEventListener("click", () => {
    changeRouter2("/crearCuenta");
  });
...
const SignIn = (userEmail, userPassword) => signInWithEmailAndPassword(auth, userEmail, userPassword);
const db = oh();
const saveTask = (nickName, email) => {
  Ql(Ta(db, "task"), { nickName, email });
};
const publicaciones = (subject2) => {
  Ql(Ta(db, "publicaciones"), { subject: subject2 });
};
const onGetNotes = (callback) => jl(Ta(db, "publicaciones"), callback);
const DeleteNotes = (id2) => Gl(Aa(db, "publicaciones", id2));
const updateNotes = (id2, newFile) => Kl(Aa(db, "publicaciones", id2), newFile);
const getNote = (id2) => Ol(Aa(db, "publicaciones", id2));
const editLike = (id2) => Ol(Aa(db, "publicaciones", id2));
const crearCuenta = (changeRouter2) => {
  ...
  BtnIr2.addEventListener("click", () => {
    ...
    nuevoUsuario(userEmail, userPassword).then((userCredential) => {
      const user = userCredential.user;
      console.log(user);
      changeRouter2("/pagPrincipal");
    }).catch((error) => {
      if (error.code === "auth/email-already-in-use") {
        document.getElementById("errorInfo").innerHTML = "Este usuario ya se encuentra registrado";
      } else if (error.code === "auth/invalid-email") {
        document.getElementById("errorInfo").innerHTML = "El correo ingresado no es v\xE1lido";
      } else if (password.length < 6) {
        document.getElementById("errorInfo").innerHTML = "Password m\xEDnimo 6 caracteres";
...
const ingresar = (changeRouter2) => {
  ...
  BtnIr.addEventListener("click", () => {
    ...
    SignIn(userEmail, userPassword).then((userCredential) => {
      ...
      if (error.code === "auth/email-already-in-use") {
        ...
        document.getElementById("errorInfo2").innerHTML = "El correo ingresado no es v\xE1lido";
      } else if (password.length < 6) {
        document.getElementById("errorInfo2").innerHTML = "Password m\xEDnimo 6 caracteres";
      } else if (error.code === "auth/weak-password") {
  ...
  Btngoogle.addEventListener("click", async () => {
    ...
    try {
      const credentials = await signInWithPopup(auth2, provider);
...
const makeWritePubli = () => {
  ...
  <textarea id="subject" name="subject" placeholder="Escribe tu Publicaci\xF3n..." ></textarea>
  </div>
